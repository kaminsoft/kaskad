...
class Post {
  ...
  static void openList(BuildContext context, bool isPublicate) {
     Navigator.of(context).push(MaterialPageRoute(
         builder: (ctx) => MessageList(isPublicate: isPublicate,)));
  }

  static void newItem(BuildContext context, {String title, String text, List<Recipient> to, bool reSend, bool isPublicate}) {
    Navigator.of(context).push(MaterialPageRoute(
        builder: (ctx) => NewItemWidget(title: title, text: text, to: to, reSend: reSend, isPublicate: isPublicate)));
  }

  static void msgSent(BuildContext context, Message msg) {
    Navigator.of(context).pop();
    var type = msg.isPublicite ? 'Объявление' : 'Сообщение';
    Toast.show('$type отправлено', context, backgroundColor: Colors.green, gravity: Toast.BOTTOM, duration: 5);
            ...
            actions: <Widget>[
              CupertinoActionSheetAction(
                  onPressed: () {
                    ...
                    Navigator.of(context).pop();
                    newItem(context, text: text, title: title,reSend: true, isPublicate: msg.isPublicite);
              ...
              CupertinoActionSheetAction(
                  onPressed: () {
                    ...
                    Navigator.of(context).pop();
                    newItem(context, text: text, title: title, to: to, isPublicate: msg.isPublicite);
                  }, child: Text("Ответить")),
              msg.to.length > 0 ? 
              CupertinoActionSheetAction(
                  onPressed: ()  async{
                    ...
                    Navigator.of(context).pop();
                    newItem(context, text: text, title: title, to: to, isPublicate: msg.isPublicite);
                  }, child: Text("Ответить всем")) : Container(),
            ],
          );
        });
  }
